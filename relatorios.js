...
const RelatoriosModule = (() => {
  ...
  async function carregar() {
    ...
    const hoje  = new Date()
    // Busca sempre 12 meses para não perder dados quando o período é menor
    const ini12 = new Date(hoje.getFullYear(), hoje.getMonth() - 11, 1)
    const start = ini12.toISOString().split('T')[0]
  ...
  function renderEvolucaoSaldo(recPorMes, despPorMes, meses) {
    ...
    el.innerHTML = meses.map(m => {
      ...
      const s = r - d
      const [yy, mm] = m.split('-')
      const nomeMes = new Date(Number(yy), Number(mm)-1, 1).toLocaleDateString('pt-BR', { month:'long' })
      const lbl = nomeMes.charAt(0).toUpperCase() + nomeMes.slice(1) + ' ' + yy
      return `
    ...
    win.document.write(`<!DOCTYPE html><html lang="pt-BR"><head>
      ...
      <table><thead><tr><th>Mês</th><th>Receitas</th><th>Despesas</th><th>Saldo</th></tr></thead><tbody>
        ${meses.map(m => {
          ...
          const [yE,mE]=m.split('-'); const nmE=new Date(Number(yE),Number(mE)-1,1).toLocaleDateString('pt-BR',{month:'long'}); const lbl=nmE.charAt(0).toUpperCase()+nmE.slice(1)+' '+yE
