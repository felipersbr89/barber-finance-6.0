...
const DespesasModule = (() => {
  ...
  async function load(){
    ...
    if (error) { Toast.err('Erro ao carregar despesas: ' + error.message); return }
    todos = data || []
  ...
  async function salvar(){
    ...
    const dataVal=document.getElementById('df-data')?.value, cat=document.getElementById('df-cat')?.value, obs=document.getElementById('df-obs')?.value.trim()
    ...
    const{error}=await window.db.from('despesas').insert({user_id:App.user.id,descricao:desc,valor,data:dataVal,categoria:cat||null,status:statusForm,observacao:obs||null})
