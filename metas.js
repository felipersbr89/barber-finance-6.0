...
const MetasModule = (() => {
  ...
  async function carregar() {
    const{data}=await window.db.from('metas').select('*').eq('user_id',App.user.id).order('created_at',{ascending:false})
    todos=data||[]
  ...
  async function salvar() {
    ...
    const{error}=await window.db.from('metas').insert({user_id:App.user.id,nome,valor_alvo:alvo,valor_atual:0,prazo:prazo||null})
  ...
  function abrirEdicao(id){const g=todos.find(x=>Number(x.id)===Number(id));if(!g)return;editandoId=id;document.getElementById('mme-nome').value=g.nome||'';document.getElementById('mme-alvo').value=g.valor_alvo||'';document.getElementById('mme-atual').value=g.valor_atual||'';document.getElementById('mme-prazo').value=g.prazo||'';Modal.open('modal-meta-edit')}
  async function salvarEdicao(){const nome=document.getElementById('mme-nome')?.value.trim(),alvo=parseFloat(document.getElementById('mme-alvo')?.value),atual=parseFloat(document.getElementById('mme-atual')?.value||'0'),prazo=document.getElementById('mme-prazo')?.value,btn=document.getElementById('mme-save-btn');if(!nome||!alvo){Toast.err('Preencha os campos.');return}Utils.setLoading(btn,true,'Salvando...');const{error}=await window.db.from('metas').update({nome,valor_alvo:alvo,valor_atual:atual,prazo:prazo||null}).eq('id',editandoId).eq('user_id',App.user.id);Utils.setLoading(btn,false,'Salvar');if(error)Toast.err('Erro: '+error.message);else{Toast.ok('Atualizado! ✓');Modal.close('modal-meta-edit');editandoId=null;await carregar()}}
  function abrirDel(id){const g=todos.find(x=>Number(x.id)===Number(id));if(!g)return;deletandoId=id;const el=document.getElementById('del-meta-desc');if(el)el.textContent='"'+g.nome+'"';Modal.open('modal-meta-del')}
  async function confirmarDel(){if(!deletandoId)return;const btn=document.getElementById('del-meta-btn');Utils.setLoading(btn,true,'Excluindo...');const{error}=await window.db.from('metas').delete().eq('id',deletandoId).eq('user_id',App.user.id);Utils.setLoading(btn,false,'Excluir');if(error)Toast.err('Erro: '+error.message);else{Toast.ok('Excluído.');Modal.close('modal-meta-del');deletandoId=null;await carregar()}}
  ...
  async function depositar(id){const meta=todos.find(x=>Number(x.id)===Number(id));if(!meta)return;const val=parseFloat(document.getElementById('dep-val-'+id).value);if(!val||val<=0){Toast.err('Valor inválido.');return}const{error}=await window.db.from('metas').update({valor_atual:Number(meta.valor_atual||0)+val}).eq('id',id).eq('user_id',App.user.id);if(error)Toast.err('Erro: '+error.message);else{Toast.ok('+'+Utils.fmt(val)+' depositado!');await carregar()}}
