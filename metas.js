...
const MetasModule = (() => {
  ...
  async function depositar(id){const meta=todos.find(x=>x.id===id);if(!meta)return;const val=parseFloat(document.getElementById('dep-val-'+id).value);if(!val||val<=0){Toast.err('Valor inválido.');return}const{error}=await window.db.from('metas').update({valor_atual:Number(meta.valor_atual||0)+val}).eq('id',id).eq('user_id',App.user.id);if(error)Toast.err('Erro: '+error.message);else{Toast.ok('+'+Utils.fmt(val)+' depositado!');await carregar()}}
